...
linkpage は、Web サイトのブックマークを管理するウェブアプリケーションです。Next.js+typescript+vitest+tailwindcss で開発しています。クライアントサーバー構成です。サーバーは next.js で RESTful 形式の API として提供されています。サーバーはローカル環境で実行することを前提にしています。クライアントは、API から取得したデータを web ブラウザ上に表示します。
